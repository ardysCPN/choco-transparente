-- =============================================================================
-- CHOCÓ TRANSPARENTE — SCRIPT CON DATOS DE PRUEBA Y DEMOSTRACIÓN (TESTING)
-- Base de datos: PostgreSQL 15+ con extensión PostGIS
-- =============================================================================

BEGIN;

-- 1. EXTENSIÓN POSTGIS Y FUNCIONES AUXILIARES
...
-- 5. CENTROS DE ACOPIO Y AUDITORÍA
...
-- 6. INVENTARIOS, LOTES, ENTRADAS Y SALIDAS
...
-- 7. BENEFICIARIOS, SOLICITUDES Y ENTREGAS
...
-- 8. DONACIONES Y CAJA TRANSPARENTE
...
-- 9. ALBERGUES, DENUNCIAS Y CONTROL SOCIAL
...
-- 31 Municipios Oficiales del Chocó
...
-- =============================================================================
-- USUARIOS DE PRUEBA Y ADMINISTRACIÓN
-- =============================================================================

-- 1. Superadministrador
-- Correo: admin@chocotransparente.gov.co | Clave: AdminChoco2026!
...
-- 2. Usuario de Testing / Prueba
-- Correo: test@chocotransparente.gov.co | Clave: TestChoco2026!
INSERT INTO public.usuario (id, nombre, apellido, correo, telefono, documento, contrasena_hash, activo, rol_id, municipio_id, organizacion_id) VALUES
...
-- Centros de Acopio de Prueba
INSERT INTO public.centro_acopio (id, municipio_id, organizacion_id, nombre, direccion, barrio, responsable, telefono, latitud, longitud, ubicacion, estado, fecha_aprobacion, aprobado_por) VALUES
(1, 1, 1, 'Almacén Central Humanitario - Quibdó', 'Calle 24 # 4-50', 'Centro', 'Lic. Roberto Palacios', '3114567890', 5.694722, -76.661111, ST_SetSRID(ST_MakePoint(-76.661111, 5.694722), 4326), 'APROBADO', NOW(), 1),
(2, 14, 1, 'Centro de Enlace Humanitario Istmina', 'Carrera 5 # 12-30', 'Cubis', 'Marta Moreno', '3128901234', 5.160000, -76.685000, ST_SetSRID(ST_MakePoint(-76.685000, 5.160000), 4326), 'APROBADO', NOW(), 1),
(3, 21, 1, 'Bodega de Acopio Costero Nuquí', 'Sector Muelle Principal', 'Pueblo Viejo', 'Carlos Murillo', '3145678901', 5.712500, -77.270833, ST_SetSRID(ST_MakePoint(-77.270833, 5.712500), 4326), 'APROBADO', NOW(), 1)
ON CONFLICT (id) DO NOTHING;

SELECT setval('centro_acopio_id_seq', (SELECT MAX(id) FROM public.centro_acopio));

-- Inventario Inicial en Centros
INSERT INTO public.inventario (id, centro_acopio_id, municipio_id, tipo_ayuda, unidad_medida, cantidad_actual, peso_actual) VALUES
(1, 1, 1, 'KITS_ALIMENTOS_FAMILIAR', 'KITS', 850, 12750),
(2, 1, 1, 'AGUA_POTABLE_TRATADA', 'LITROS', 5000, 5000),
(3, 1, 1, 'KITS_ASEO_PERSONAL', 'KITS', 620, 3100),
(4, 2, 14, 'KITS_ALIMENTOS_FAMILIAR', 'KITS', 400, 6000),
(5, 3, 21, 'COLCHONETAS_FRAZADAS', 'UNIDADES', 300, 1500)
ON CONFLICT (id) DO NOTHING;

SELECT setval('inventario_id_seq', (SELECT MAX(id) FROM public.inventario));

-- Albergues de Emergencia
INSERT INTO public.albergue (id, municipio_id, nombre, direccion, latitud, longitud, ubicacion, capacidad, ocupacion, responsable, telefono, servicios, estado) VALUES
(1, 1, 'Coliseo Municipal El Jardín', 'Barrio El Jardín, Quibdó', 5.698000, -76.658000, ST_SetSRID(ST_MakePoint(-76.658000, 5.698000), 4326), 250, 85, 'Dra. Sandra Mena', '3157890123', 'Agua potable, comedor comunitario, baños, enfermería', 'DISPONIBLE'),
(2, 14, 'Institución Educativa San Pablo', 'Barrio San Pablo, Istmina', 5.165000, -76.682000, ST_SetSRID(ST_MakePoint(-76.682000, 5.165000), 4326), 120, 45, 'Prof. Emiro Rentería', '3168901234', 'Alojamiento, cocina comunal, agua', 'DISPONIBLE'),
(3, 24, 'Centro Comunal Riosucio Ribera', 'Sector La Playa, Riosucio', 7.442000, -77.118000, ST_SetSRID(ST_MakePoint(-77.118000, 7.442000), 4326), 80, 70, 'Jhonatan Perea', '3179012345', 'Primeros auxilios, raciones', 'CASI_LLENO')
ON CONFLICT (id) DO NOTHING;

SELECT setval('albergue_id_seq', (SELECT MAX(id) FROM public.albergue));

-- Afectaciones
INSERT INTO public.afectacion (id, municipio_id, nombre, descripcion, tipo, severidad, estado, latitud, longitud, ubicacion, direccion, fecha_inicio, creado_por) VALUES
(1, 1, 'Inundación Cuenca Media Río Atrato', 'Desbordamiento por lluvias torrenciales afectando zonas ribereñas de Quibdó', 'INUNDACION', 'ALTA', 'ACTIVA', 5.694722, -76.661111, ST_SetSRID(ST_MakePoint(-76.661111, 5.694722), 4326), 'Barrios San Vicente y El Caraño', CURRENT_DATE - INTERVAL '3 days', 1),
(2, 14, 'Creciente Súbita Río San Juan', 'Afectación a viviendas por creciente súbita en el sector del muelle fluvial', 'INUNDACION', 'MEDIA', 'ACTIVA', 5.160000, -76.685000, ST_SetSRID(ST_MakePoint(-76.685000, 5.160000), 4326), 'Ribera Río San Juan, Istmina', CURRENT_DATE - INTERVAL '2 days', 1),
(3, 9, 'Deslizamiento Vía Quibdó - Medellín', 'Pérdida de banca y derrumbe que incomunica el transporte terrestre', 'DESLIZAMIENTO', 'CRITICA', 'EN_ATENCION', 5.604722, -76.143889, ST_SetSRID(ST_MakePoint(-76.143889, 5.604722), 4326), 'Sector El Siete, El Carmen de Atrato', CURRENT_DATE - INTERVAL '1 days', 1)
ON CONFLICT (id) DO NOTHING;

SELECT setval('afectacion_id_seq', (SELECT MAX(id) FROM public.afectacion));

-- Donaciones de Prueba
INSERT INTO public.donacion (id, organizacion_id, tipo, donante, monto, descripcion, municipio_id, estado) VALUES
(1, 1, 'DINERO', 'Fundación Solidaridad por Colombia', 25000000, 'Aporte para compra de kits humanitarios', 1, 'CONFIRMADA'),
(2, 1, 'ESPECIE', 'Asociación de Comerciantes de Antioquia', NULL, 'Donación de 200 kits de aseo y frazadas', 1, 'CONFIRMADA')
ON CONFLICT (id) DO NOTHING;

SELECT setval('donacion_id_seq', (SELECT MAX(id) FROM public.donacion));

INSERT INTO public.donacion_dinero (donacion_id, cuenta_destino, referencia, monto, soporte_archivo) VALUES
(1, '458-920194-11', 'REF-BCOL-89214', 25000000, 'https://ejemplo.com/comprobante_donacion.pdf')
ON CONFLICT (donacion_id) DO NOTHING;

-- Gastos Auditados de Prueba
INSERT INTO public.gasto (id, organizacion_id, concepto, monto, proveedor, numero_factura, fecha, soporte, estado, creado_por, aprobado_por) VALUES
(1, 1, 'Adquisición de combustible fluvial para transporte de ayudas', 3800000, 'Distribuidora de Combustibles Atrato SAS', 'FE-98214', CURRENT_DATE - INTERVAL '1 days', 'https://ejemplo.com/factura_electronica_98214.pdf', 'APROBADO', 1, 1)
ON CONFLICT (id) DO NOTHING;

SELECT setval('gasto_id_seq', (SELECT MAX(id) FROM public.gasto));
